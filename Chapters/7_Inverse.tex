\label{inverse_section}
	...
	\begin{algorithm}[H]
		\NoCaptionOfAlgo
		\SetAlgoLined
			construct customers versus products matrix using transactions provided by the Recommender\;
			create a Graph object g by giving customers versus products matrix as a parameter\;
			\If{Proposed Method 1 has been chosen}{
				construct similarity matrix based on cosine similarity between customers\;
				construct weight matrix using the trust matrix of the g and the similarity matrix\;
			}\Else{
				weight matrix equals to the trust matrix of the g\;
			}
			\ForEach{customer c}{
				choose n customers that c trusts most as neighbors\;
				\ForEach{product p purchased by the neighbours}{
					\If{c purchased p}{
						continue;
					}\Else{
